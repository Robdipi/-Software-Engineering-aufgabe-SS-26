...
class ControllerV2Spec extends AnyWordSpec with Matchers {
  ...
  "ControllerV2" should {
    ...
    "warn when the current player cannot afford the requested card" in {
      val (controller, observer, _, _) = newController()
      val game = Gamestate(
        Players = List(player(0, money = 0)),
        CurrentTurnPlayerId = 0,
      ...
      warningBeforeBuyphase(observer) shouldBe YOU_CANT_AFFORD_THIS_WARNING
    }

    "warn when a yellow card is bought twice" in {
      val (controller, observer, _, _) = newController()
      val game = Gamestate(
      ...
      warningBeforeBuyphase(observer) shouldBe ALREADY_OWN_THAT_YELLOW_CARD_WARNING
    }

    "warn when a second purple card is bought" in {
      val (controller, observer, _, _) = newController()
      val game = Gamestate(
        Players = List(player(0, money = 20, cards = List(stadion))),
        CurrentTurnPlayerId = 0,
        cardStacks = List(cardStack(1, fernsehsender))
      )

      controller.handleInput(BuyCardInput("Fernsehsender"), game)

      warningBeforeBuyphase(observer) shouldBe ALREADY_OWN_PURPLE_CARD_WARNING
    }

    "warn when a matching stack is empty" in {
      val (controller, observer, _, _) = newController()
      val game = Gamestate(
      ...
      controller.handleInput(BuyCardInput("Weizenfeld"), game)

      warningBeforeBuyphase(observer) shouldBe NO_CARDS_LEFT_OF_THAT_TYPE_WARNING
    }

    "warn for an unknown card name" in {
      val (controller, observer, _, _) = newController()
      val game = Gamestate(Players = List(player(0)), CurrentTurnPlayerId = 0)

      controller.handleInput(BuyCardInput("does-not-exist"), game)

      warningBeforeBuyphase(observer) shouldBe NONE_EXISTANT_CARDNAME_WARNING
    }


    "reject malformed undo input as an unknown card" in {
      val (controller, observer, undo, _) = newController()
      val game = Gamestate(Players = List(player(0)), CurrentTurnPlayerId = 0)

      controller.handleInput(BuyCardInput("undo"), game)

      undo.undoCalls shouldBe empty
      warningBeforeBuyphase(observer) shouldBe NONE_EXISTANT_CARDNAME_WARNING
    }

    "warn instead of throwing when the current player is missing" in {
      val (controller, observer, _, _) = newController()
      val game = Gamestate(
        Players = List(player(0, money = 10)),
